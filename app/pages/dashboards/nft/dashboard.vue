<script setup>
import * as NftData from "~/data/dashboards/nft/dashboarddata";
const dataToPass = {
    ...
    <div class="col-xxl-9">
        <div class="row">
                ...
                <div class="card custom-card nft-main-banner-card border-0">
                    <div class="card-body">
                        <div class="nft-main-bannr-background">
                            <img src="/images/nft-images/1.png" alt="" class="d-md-block d-none">
                        </div>
                        <h2 class="text-fixed-white">Discover, buy, and sell unique NFTs</h2>
                        <p class="fs-16 op-8 mb-4">Immerse yourself in a world where digital creativity <br> meets
            ...
            <div class="d-flex align-items-center gap-4 mb-4 flex-wrap">
                <div :class="`nft-tag nft-tag-${idx.className} ${idx.isActive ? 'active' : ''}`" v-for="(idx) in NftData.NftTags" :key="idx.id">
                    <a href="javascript:void(0);" class="stretched-link"></a>
                    <span class="nft-tag-text">{{ idx.label }}</span>
                </div>
            </div>
            <div class="col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-6 col-12" v-for="(idx) in NftData.NftCards" :key="idx.id">
                ...
                <div class="card custom-card">
                    <div class="card-body">
                        ...
                        <div id="nft-balance-chart">
                            <apexchart height="60px" type="area" :options="NftData.BalanceOptions" :series="NftData.BalanceSeries"></apexchart>
                        </div>
                ...
                <div class="card custom-card overflow-hidden">
                    ...
                    <div class="card-body">
                        <ul class="list-unstyled nft-top-collectors-list">
                            <li v-for="(idx) in NftData.Followers" :key="idx.id">
                                <div class="d-flex align-items-center gap-2 flex-wrap">
                                    ...
                                    <div class="flex-fill">
                                        <span class="fw-semibold d-block">{{ idx.name }}<i class="bi bi-patch-check-fill text-primary align-middle ms-2"></i></span>
                                        <span class="text-muted fs-13">{{ idx.handle }}</span>
                                    </div>
                                    <div>
                                        <button :class="`btn btn-${idx.isFollowing ? 'primary' : 'light'} rounded-pill`">
                                            {{ idx.isFollowing ? 'Following' : 'Follow' }}</button>
                                    </div>
                                </div>
                            </li>
                        </ul>
        ...
        <div class="card custom-card">
            ...
            <div class="card-body pb-2">
                ...
                <div id="nft-statistics">
                    <apexchart height="280px" type="bar" :options="NftData.NftOptions" :series="NftData.NftSeries">
                    </apexchart>
                </div>
            </div>
        </div>
        ...
        <div class="card custom-card">
            ...
            <div class="card-body">
                <ul class="list-unstyled nft-top-drops-list">
                    <li v-for="(idx) in NftData.NftSales" :key="idx.id">
                        <div class="d-flex align-items-center gap-2">
                            <div class="lh-1">
                                <span class="avatar avatar-md avatar-rounded">
                                    <img :src="idx.image" alt="">
                            ...
                            <div class="flex-fill">
                                ...
                                <span class="fs-13 text-muted">By {{ idx.author }}</span>
                            </div>
                            <div class="text-end">
                                <div class="d-flex align-items-center gap-1">
                                    <div class="lh-1">
                                        <span class="avatar avatar-xs">
                                            <img src="/images/crypto-currencies/crypto-icons/ethereum-eth-logo.svg" alt="">
                                        </span>
                                    </div>
    ...
    <div class="col-xxl-3 col-xl-6">
        <div class="card custom-card">
            ...
            <div class="card-body p-2">
                <SwiperjsCards :swiperItems="NftData.NftCardsData" swiperClass="swiper pagination-dynamic text-start featured-collections" :slidesPerView="1" :spaceBetween="20" :pagination="false" :navigation="false" :autoplay="true">
                    <template #default="{ card }">
                        <SpkNftSwipercard :card="card" />
                    </template>
                </SwiperjsCards>
            </div>
        </div>
        ...
        <div class="card custom-card">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <TableComponent tableClass="table text-nowrap" :headers="[{ text: 'S.No', thClass: '' }, { text: 'NFT Name', thClass: '' }, { text: 'Creator', thClass: '' }, { text: 'Release Date', thClass: '' }, { text: 'Price', thClass: '' }, { text: 'Status', thClass: '' }, { text: 'Category', thClass: '' }, { text: 'Blockchain', thClass: '' }, { text: 'Ownership', thClass: '' }, { text: 'Volume (ETH)', thClass: '' },]" :rows="NftData.NftTableData" v-slot:cell="{ row }">
                        <td>{{ row.id }}</td>
                        <td>
                            <div class="d-flex align-items-center gap-2">
                                <div class="lh-1">
                                ...
                                <div>
                                    ...
                                    <span class="fs-13 text-muted">{{ row.handle }}</span>
                                </div>
                            </div>
                        </td>
                        <td>{{ row.creator }}</td>
                        <td>{{ row.date }}</td>
                        <td>{{ row.price }}</td>
                        <td><span :class="`badge bg-${row.statusClass}-transparent`">{{ row.status }}</span></td>
                        <td>{{ row.category }}</td>
                        <td>{{ row.chain }}</td>
                        <td>{{ row.edition }}</td>
                        <td>{{ row.volume }}</td>
